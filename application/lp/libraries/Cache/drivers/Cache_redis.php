...
class CI_Cache_redis extends CI_Driver
{
	...
	{
		$redius_config = get_config();
		self::$_default_config = array(
			'host' => $redius_config['redis_host'],
			'password' => $redius_config['redis_password'],
			'port' => $redius_config['redis_port'],
			'timeout' => $redius_config['redis_timeout']
		);	
		$this->is_supported();
	}
	 ...
	 * @return	bool
	 */
	public function save($key, $value, $ttl = false)
	{
		if($this->_redis) {
			return $ttl ? $this->_redis->setex($key, $ttl, serialize($value)) : $this->_redis->set($key, serialize($value));
		}
	}
